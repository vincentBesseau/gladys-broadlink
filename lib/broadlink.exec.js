...
module.exports = function(params) {
	...
	if(params.deviceType.unit === 'MP1') {
		if(!shared.instances.mp1Instances[params.deviceType.device]) return reject(`No broadlink with deviceId ${params.deviceType.device}`);
		...
		if(!shared.instances.rm2Instances[params.deviceType.device]) return reject(`No broadlink with deviceId ${params.deviceType.device}`);
