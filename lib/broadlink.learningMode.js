...
module.exports = function learningMode(options) {
    ...
    .then((machineUuid) => {
        gladys.device.getByIdentifier({identifier:'rm2',service:'gladys-broadlink'})
        .then((device) => {
            ...
            {
                var json = '{"_module":"RM2","_type":"learning","_name":"'+devicetypeName+'","_deviceId":"'+deviceId+'"}'
                var topic = 'gladys/machine/'+machineId+'/module/gladys-broadlink/notify'
                gladys.modules.mqtt.emit(topic,json)
                return Promise.resolve('Broadcast : Device '+devicetypeName+' create')
            } else {
                return Promise.resolve('Broadcast : Error to enter on learning mode !')
            }
        })
    })
}
